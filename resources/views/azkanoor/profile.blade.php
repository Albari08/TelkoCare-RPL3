            ...
            <div class="hidden" id="styled-dokter" role="tabpanel" aria-labelledby="dokter-tab">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-5 md:gap-x-24 md:gap-y-14 ">
                    @foreach ($dokters as $dokter)
                        <a href="{{ route('admin.jadwal.dokter', ['id' => $dokter->id]) }}">
                            <div
                                class="flex gap-3 items-center justify-between px-10 py-5 border-b border-[#FF0000] bg-white">
            ...
            <div class="hidden" id="styled-pasien" role="tabpanel" aria-labelledby="pasien-tab">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-x-24 md:gap-y-14">
                    @foreach ($pasiens as $pasien)
                        <a href="{{ route('admin-profile-pasien-detail', ['id' => $pasien->id]) }}">
                            <div
                                class="flex gap-3 items-center justify-between px-10 py-5 border-b border-[#FF0000] h-full bg-white">
                                <img src="{{ asset('asset/img/dokter.png') }}" alt="dokter">
                                <div class="text-center">
                                    <h2 class="font-bold text-xl">{{ $pasien->nama }}</h2>
                                    <p class="text-sm">
                                        {{ \Carbon\Carbon::parse($pasien->tanggal_lahir)->translatedFormat('l, j F Y') }}
                                    </p>
                                </div>
                                <div>
                                    <svg width="8" viewBox="0 0 10 16" fill="none"
