...
<body class="bg-white font-family-karla flex">
    ...
    <div class="flex-1 p-5">
        <h1 class="font-bold text-4xl text-gray-600">Resep Dokter</h1>
        ...
        <div class="overflow-x-auto mt-5">
            <table class="table-auto w-full border-separate border-spacing-y-2">
              ...
              <tbody>
                ...
                <tr>
                    <td class="px-4 py-2 border border-gray-700 border-r-0 rounded-tl-lg rounded-bl-lg">{{ $row->kode_resep }}</td>
                    <td class="px-4 py-2 border border-gray-700 border-x-0 ">{{ date('d-m-Y', strtotime($row->created_at)) }}</td>
                    <td class="px-4 py-2 border border-gray-700 border-x-0">{{ $row->nama_pasien }}</td>
                    <td class="px-4 py-2 border border-gray-700 border-x-0">{{ $row->status }}</td>
                    <td class="px-4 py-2 border border-gray-700 border-l-0 rounded-tr-lg rounded-br-lg">
                      <a href="{{ route('resep.show', $row->id )}}" class="bg-red-500 text-white px-4 py-1 rounded-xl text-sm">Rincian</a>
                    </td>
                </tr>
                @endforeach
              </tbody>
